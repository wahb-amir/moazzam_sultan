...
const Portfolio = (props) => {
  const {
    ...
    containerClassName = "py-24 bg-white",
    modalAutoplay = true,
  } = props;
    ...
    <section id="portfolio" className={containerClassName}>
      <div className="container mx-auto px-6">
        ...
        <div className="flex gap-4 mb-12 overflow-x-auto pb-4 no-scrollbar">
          {Array.from(new Set(["all", ...items.map((i) => i.type)])).map(
            (type) => (
              <button
                key={type}
                onClick={() => setFilter(type)}
                className={`px-8 py-3 rounded-2xl text-sm font-bold transition-all border whitespace-nowrap ${filter === type ? "bg-slate-900 text-white border-slate-900 shadow-lg" : "bg-white text-slate-500 border-slate-100 hover:border-blue-200"}`}
              >
                {type === "all"
                  ? "Everything"
                  : type === "intro"
                ...
                >
                  <div className="w-full lg:w-[65%] bg-black flex items-center">
                    <div className="w-full aspect-video">
                      <iframe
                        src={`https://www.youtube.com/embed/${getYouTubeId(selectedItem.videoUrl)}?autoplay=${modalAutoplay ? 1 : 0}`}
