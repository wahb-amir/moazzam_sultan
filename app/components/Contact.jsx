  ...
  return (
    <section id="contact" className="py-24 bg-white text-slate-900 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.4] pointer-events-none" 
           style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23f1f5f9' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")` }} 
      />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          <div className="lg:sticky lg:top-24">
            <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-100 rounded-full mb-6">
              <Sparkles size={14} className="text-blue-600" />
              <span className="text-blue-700 text-xs font-bold uppercase tracking-widest">Available for Demo</span>
            </motion.div>
            
            <motion.h2 variants={itemVariants} className="text-5xl md:text-7xl font-black mb-8 leading-[1.1] text-slate-900 tracking-tight">
              Ready to Master <br /> 
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Mathematics?</span>
            </motion.h2>
            
            <motion.p variants={itemVariants} className="text-slate-600 mb-12 text-lg md:text-xl leading-relaxed max-w-md font-medium">
              Struggling with Calculus or Algebra? Reach out and let’s make Math your favorite subject.
            </motion.p>

            <div className="space-y-4">
              {[
                { icon: Phone, label: "WhatsApp", val: "+92 309 7016696", link: SOCIAL_LINKS.whatsapp, color: "hover:border-green-200", iconBg: "bg-green-50 text-green-600" },
                { icon: Mail, label: "Email Me", val: "sultanmoazzam3@gmail.com", link: `mailto:sultanmoazzam3@gmail.com`, color: "hover:border-blue-200", iconBg: "bg-blue-50 text-blue-600" }
              ].map((item, i) => (
                <motion.a 
                  key={i}
                ...
                >
                  ...
                  <ExternalLink className="ml-auto text-slate-300 group-hover:text-blue-600 transition-colors" size={20} />
                </motion.a>
              ))}
            </div>
          </div>
          ...
          <motion.div variants={itemVariants}>
            <div className="bg-white/90 backdrop-blur-md rounded-[3rem] p-8 md:p-12 border border-slate-100 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)]">
              <h3 className="text-2xl font-bold mb-10 text-slate-900 flex items-center gap-3">
                Send a Message <MessageCircle className="text-blue-600" size={24} />
              </h3>
              ...
              <form className="space-y-7" onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2 contact-phone-input">
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-2">Phone</label>
                    <PhoneInput
                      country={'pk'}
                      value={phone}
                      onChange={setPhone}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center ml-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Your Inquiry</label>
                    <span className={`text-[10px] font-bold ${formData.message.length === CHARACTER_LIMIT ? 'text-red-500' : 'text-slate-400'}`}>
                ...
                <button 
                  ...
                  className={`w-full h-16 rounded-2xl font-black uppercase tracking-[0.2em] text-sm relative overflow-hidden transition-all transform active:scale-95 shadow-lg ${
                    formState === "sent" ? "bg-green-500 text-white" : 
                ...
                >
                  <AnimatePresence mode="wait">
                    {formState === "idle" && (
                      <motion.div key="idle" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex items-center justify-center gap-3">
                        <Send size={18} /> Send Inquiry
                      </motion.div>
                    )}
                    {formState === "sending" && (
                      <motion.div key="sending" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center">
                        <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin" />
                      </motion.div>
                    )}
                    {formState === "sent" && (
                      <motion.div key="sent" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex items-center justify-center gap-2">
                        <CheckCircle2 size={20} /> Message Sent Successfully!
                      </motion.div>
                    )}
                    {formState === "error" && (
                      <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-center gap-2">
